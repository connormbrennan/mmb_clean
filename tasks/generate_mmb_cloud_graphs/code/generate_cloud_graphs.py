"""
Purpose:
    Generate MMB IRF cloud graphs from the constructed IRF panel.
...
Outputs:
    ../output/*.pdf
    ../output/*_description.txt
    ../output/cloud_graphs_manifest.txt

Run:
    make from tasks/generate_mmb_cloud_graphs/code/
"""
...
for file_base, var_name, rule_value, type_column, subtitle in plot_specs:
    ...
    figure_path = output_dir / f"{file_base}.pdf"
    fig.savefig(figure_path, bbox_inches="tight")
    plt.close(fig)

    description_path = output_dir / f"{file_base}_description.txt"
    with open(description_path, "w", encoding="utf-8") as f:
        ...
        f.write("Data source: tasks/build_mmb_analysis_dataset/output/MMB_IRF_format_full.dta.\n")
        if subtitle:
...
manifest.to_csv(output_dir / "cloud_graphs_manifest.txt", index=False)
print(f"Wrote {len(manifest)} cloud graphs to {output_dir}.")
