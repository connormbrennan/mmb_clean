...
"""
Purpose:
  Build nominal-rigidity regression tables for the MMB outcomes.
...
def build_manuscript_table(sample_name, sample_label, sample_df, citation_weighted=False):
    ...
    caption = "Effects of Model Nominal Rigidities on Macro Outcomes (full sample)"
    label = "tab:nominal_rigidities"
    note_tail = "See Table~\\ref{tab:nominal_rigidities_B} for statistics with estimated models only."
    output_stem = "table7_nominal_rigidities_full_sample"
    if sample_name == "estimated_models":
        caption = "Effects of Model Nominal Rigidities on Macro Outcomes (estimated models only)"
        label = "tab:nominal_rigidities_B"
        note_tail = "See Table~\\ref{tab:nominal_rigidities} for full-sample statistics."
        output_stem = "table7_nominal_rigidities_estimated_models"
    if citation_weighted:
    ...
    for row_index, (term, label_text) in enumerate(display_terms):
        ...
        if row_index < len(display_terms) - 1:
        ...
        manifest_rows.append(
            {
                ...
                "sample": sample_name,
                "citation_weighted": citation_weighted,
                "description": f"Manuscript LaTeX{weight_label} Table 7 for {sample_label}.",
            }
        )
        manifest_rows.append(
            {
                ...
                "sample": sample_name,
                "citation_weighted": citation_weighted,
                "description": f"Source values for manuscript{weight_label} Table 7 for {sample_label}.",
            }
        )
...
manifest_rows.append(
    {
        ...
        "sample": "all",
        "citation_weighted": False,
        "description": "Manifest of nominal-rigidities task outputs.",
    }
)
