...
"""
Purpose:
  Build real-rigidity regression tables for the MMB outcomes.
...
SPEC_KEY = "real_rigidities"

params = load_table_params(__file__)
spec = params["table_specs"][SPEC_KEY]
estimated_spec = params["table_specs"]["real_rigidities_estimated"]
...
def identifying_support(term, data, depvar, variables):
    ...
    elif term in REAL_RIGIDITY_VARS:
...
def write_text_summary(path, title, sample_name):
    ...
    if sample_name == "estimated_models":
        spec_note = "The estimated-model appendix uses main effects only because the all-pairwise real-rigidity specification has sparse model-level support in this subsample."
    else:
        spec_note = "The full-sample table uses the all-pairwise real-rigidity specification."

    lines = [
        title,
        "",
        spec_note,
        "Use the Table 8 LaTeX and CSV outputs for support-suppressed manuscript values.",
        "Use the companion support CSVs and table8_real_rigidities_identification_diagnostics.csv to inspect model-level support and dropped design columns.",
        "",
...
def build_manuscript_table(sample_name, sample_label, sample_df, citation_weighted=False):
    ...
    caption = "Effects of Model Real Rigidities on Macro Outcomes (full sample)"
    label = "tab:real_rigidities"
    note_tail = "See Table~\\ref{tab:real_rigidities_B} for statistics with estimated models only."
    output_stem = "table8_real_rigidities_full_sample"
    if sample_name == "estimated_models":
        caption = "Effects of Model Real Rigidities on Macro Outcomes (estimated models only)"
        label = "tab:real_rigidities_B"
        note_tail = "See Table~\\ref{tab:real_rigidities} for full-sample statistics."
        output_stem = "table8_real_rigidities_estimated_models"
    if citation_weighted:
        ...
        manifest_rows.append(
            {
                ...
                "sample": sample_name,
                "citation_weighted": citation_weighted,
                "description": f"Manuscript LaTeX{weight_label} Table 8 for {sample_label}.",
            }
        )
        manifest_rows.append(
            {
                ...
                "sample": sample_name,
                "citation_weighted": citation_weighted,
                "description": f"Source values for manuscript{weight_label} Table 8 for {sample_label}.",
            }
        )
        manifest_rows.append(
            {
                ...
                "sample": sample_name,
                "citation_weighted": citation_weighted,
                "description": f"Coefficient-cell support diagnostics for manuscript{weight_label} Table 8 for {sample_label}.",
            }
        )

identification_path = OUTPUT_DIR / "table8_real_rigidities_identification_diagnostics.csv"
...
manifest_rows.append(
    {
        ...
        "sample": "all",
        "citation_weighted": False,
        "description": "Identification diagnostics listing original, kept, and dropped design columns for Table 8 regressions.",
    }
)
manifest_rows.append(
    {
        ...
        "sample": "all",
        "citation_weighted": False,
        "description": "Manifest of real-rigidities task outputs.",
    }
)
